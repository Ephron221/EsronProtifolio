...
const DocumentViewer = () => {
  ...
  function onDocumentLoadError(error: Error): void {
    ...
    setPdfError(`Failed to render document preview. (${error.message})`);
  }
  
  const fileUrl = document?.fileUrl ? getAssetUrl(document.fileUrl) : '';
  ...
  const renderContent = () => {
    if (isLoading) {
      return (
        <div className="py-24 flex flex-col items-center justify-center gap-3">
          <LoadingSpinner />
          <span className="text-xs font-semibold text-gray-400">Loading secure document...</span>
        </div>
      );
    }
    ...
    if (isError) {
      return (
        <div className="w-full h-full flex flex-col items-center justify-center text-red-500 p-8 text-center">
          ...
          <p className="text-xs text-red-400 mt-1">{error instanceof Error ? error.message : 'Document not found or inaccessible.'}</p>
        </div>
      );
    }
      ...
      return (
        <Document
          ...
          loading={
            <div className="py-24 flex flex-col items-center justify-center gap-3">
              <LoadingSpinner />
              <span className="text-xs font-semibold text-gray-400">Rendering high-resolution canvas...</span>
            </div>
          }
        ...
        >
          {Array.from(new Array(numPages || 0), (_, index) => (
            <div key={`page_${index + 1}`} className="mb-6 relative group shadow-2xl rounded-xl overflow-hidden">
              <Page
                pageNumber={index + 1}
    ...
    return (
      <div className='w-full h-full flex flex-col items-center justify-center text-gray-400 p-8 text-center'>
        ...
        <p className='font-bold text-base text-gray-800 dark:text-gray-200'>Document Protected</p>
        <p className='text-xs text-gray-500 mt-1 max-w-sm'>
          Unable to locate document file or URL is restricted.
        </p>
      </div>
    );
  };
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tight">
              {document?.title || 'Secured Credential'}
            </h1>
        ...
        <div className="p-3.5 bg-primary/5 border border-primary/15 rounded-2xl flex items-center gap-3 text-xs">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            <strong className="text-gray-900 dark:text-white">Protected Document:</strong> Downloading, printing, and extraction are restricted across all devices. Use zoom controls to inspect certificate details.
          </p>
        </div>
        ...
        <div className="relative group">
          <motion.div
            layout
            ref={pdfContainerRef}
            className="relative bg-zinc-100 dark:bg-zinc-950 rounded-[32px] overflow-auto shadow-2xl border-4 border-gray-100 dark:border-white/5 transition-all duration-500 min-h-[70vh] max-h-[82vh] custom-scrollbar flex justify-center p-2 sm:p-6"
          >
            {/* Transparent Protection Shield (Blocks direct drag / right clicks) */}
            <div className="absolute inset-0 z-30 pointer-events-none" onContextMenu={(e) => e.preventDefault()} />

            {/* Subtle Anti-Tamper Security Watermark */}
            <div className="absolute inset-0 z-20 pointer-events-none opacity-[0.03] dark:opacity-[0.04] flex flex-wrap gap-20 p-16 overflow-hidden rotate-[-18deg] select-none">
              {[...Array(24)].map((_, i) => (
                <span key={i} className="text-4xl font-black whitespace-nowrap text-black dark:text-white uppercase tracking-widest">
                  ESRON VERIFIED CREDENTIAL
                </span>
              ))}
            </div>
            ...
            <div className="absolute bottom-6 right-6 z-40 px-4 py-2 bg-black/60 dark:bg-black/80 backdrop-blur-xl rounded-full border border-primary/30 flex items-center gap-2">
